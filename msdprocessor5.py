...
class msdProcessor(processor.ProcessorABC):
    ...
    def process(self, events, beta, beta_min, z_cut, z_cut_min, n):
        #Main function call

        #Set global variables from process()
        self.n = n
        ...
        # 0.8 Defines Ro, a wide-jet softdrop condition. Note this wide-jet technique 
        ...
        for i in range(n):
            for j in range(n):
                ...
                beta_softdrop_matrix[i][j] = cluster.exclusive_jets_softdrop_grooming(
                    beta=(beta*(i/n) + beta_min), symmetry_cut= (z_cut*(j/n) + z_cut_min)
                )
